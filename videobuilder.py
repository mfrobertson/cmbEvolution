...
class Video:
    ...
    def buildVideo(self):
        ...
        os.chdir(os.getcwd())
        os.chdir(self.loc)
        os.system(fr'ffmpeg -i {picDir}\field_%02d.png -filter:v "setpts=2*PTS" -r 30 -pix_fmt yuv420p -y {self.name}.mp4')

        return picDir

    def drawPics(self, fr):
        dir = fr"{self.loc}\pics_{self.N}_{self.scale}"
        ...
        for iii, eta in enumerate(self.etas):
            ...
            plt.savefig(rf"{dir}\field_%02d.png" % eta, bbox_inches='tight', pad_inches=0)
